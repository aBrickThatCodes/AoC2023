...
{
    #region Part 1
    static bool IsSchematicSymbol(this char c)
    {
    ...
    static int Part1(string[] lines)
    {
        ...
        {
            int beginNumber = -1, endNumber = -1, leftmost = 0;
            ...
            for (var j = 0; j < lineLength; j++)
            {
                ...
                if ((!currentIsDigit || lineEnd) && (beginNumber != -1 && endNumber == -1))
                {
                    endNumber = currentIsDigit ? j : j - 1;
                    var isPart = false;
                    ...
                    if (!isPart && leftmost != beginNumber)
                    {
                        isPart = IsSchematicSymbol(currentLine[leftmost]);
                    }

                    if (!isPart && endNumber != j)
                    {
                        isPart = IsSchematicSymbol(currentLine[j]);
                    }
    ...
    static void AddToGear(this Dictionary<(int, int), Gear> gears, (int, int) key, int n)
    {
        if (!gears.TryGetValue(key, out Gear? gear))
        {
            gear = new Gear(n);
            gears.Add(key, gear);
        }
    ...
    static int Part2(string[] lines)
    {
        ...
        var gearRatios = from gear in gears.Values where gear.n == 2 select gear.ratio;
